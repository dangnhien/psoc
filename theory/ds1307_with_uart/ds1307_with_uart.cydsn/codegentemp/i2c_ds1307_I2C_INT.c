...
/*******************************************************************************
* Function Name: i2c_ds1307_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(i2c_ds1307_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef i2c_ds1307_I2C_ISR_ENTRY_CALLBACK
    i2c_ds1307_I2C_ISR_EntryCallback();
#endif /* i2c_ds1307_I2C_ISR_ENTRY_CALLBACK */

#if (i2c_ds1307_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = i2c_ds1307_I2C_ACK_ADDR;
    ...
    if(NULL != i2c_ds1307_customIntrHandler)
    {
        i2c_ds1307_customIntrHandler();
    }

    if(i2c_ds1307_CHECK_INTR_I2C_EC_MASKED(i2c_ds1307_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(i2c_ds1307_CHECK_I2C_FSM_MASTER)
    {
        #if(i2c_ds1307_I2C_MASTER)
        {
            ...
            */
            if(i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_BUS_ERROR))
            {
                i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_ERR_XFER |
                                                         i2c_ds1307_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = i2c_ds1307_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_ARB_LOST))
            {
                i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_ERR_XFER |
                                                         i2c_ds1307_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = i2c_ds1307_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(i2c_ds1307_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(i2c_ds1307_CHECK_I2C_MASTER_CMD(i2c_ds1307_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_ERR_XFER |
                                                             i2c_ds1307_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                i2c_ds1307_mstrStatus |= (uint16) i2c_ds1307_GET_I2C_MSTAT_CMPLT;
                ...
                #if(i2c_ds1307_I2C_MULTI_MASTER_SLAVE)
                {
                    if(i2c_ds1307_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(i2c_ds1307_CHECK_I2C_MASTER_CMD(i2c_ds1307_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            i2c_ds1307_I2C_MASTER_CLEAR_START;

                            endTransfer = i2c_ds1307_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!i2c_ds1307_CHECK_INTR_SLAVE_MASKED(i2c_ds1307_INTR_SLAVE_I2C_BUS_ERROR))
                               && i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = i2c_ds1307_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            i2c_ds1307_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            i2c_ds1307_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            i2c_ds1307_state = i2c_ds1307_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (i2c_ds1307_CHECK_I2C_FSM_SLAVE) */
    {
        #if(i2c_ds1307_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or i2c_ds1307_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(i2c_ds1307_CHECK_INTR_SLAVE_MASKED(i2c_ds1307_INTR_SLAVE_I2C_BUS_ERROR |
                                                        i2c_ds1307_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(i2c_ds1307_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    i2c_ds1307_slStatus &= (uint8) ~i2c_ds1307_I2C_SSTAT_RD_BUSY;
                    i2c_ds1307_slStatus |= (uint8) (i2c_ds1307_I2C_SSTAT_RD_ERR |
                                                          i2c_ds1307_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    i2c_ds1307_slStatus &= (uint8) ~i2c_ds1307_I2C_SSTAT_WR_BUSY;
                    i2c_ds1307_slStatus |= (uint8) (i2c_ds1307_I2C_SSTAT_WR_ERR |
                                                          i2c_ds1307_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(i2c_ds1307_CHECK_I2C_FSM_MASTER)
    {
        #if(i2c_ds1307_I2C_MASTER)
        {
            ...
            */
            if(i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_STOP))
            {
                i2c_ds1307_ClearMasterInterruptSource(i2c_ds1307_INTR_MASTER_I2C_STOP);
            ...
            {
                if(i2c_ds1307_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_NACK))
                    {
                        i2c_ds1307_ClearMasterInterruptSource(i2c_ds1307_INTR_MASTER_I2C_NACK);

                        i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_ERR_XFER |
                                                                 i2c_ds1307_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(i2c_ds1307_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != i2c_ds1307_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                i2c_ds1307_SetTxInterruptMode(i2c_ds1307_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(i2c_ds1307_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(i2c_ds1307_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(i2c_ds1307_CHECK_INTR_RX_MASKED(i2c_ds1307_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  i2c_ds1307_mstrRdBufSize -
                                        (i2c_ds1307_mstrRdBufIndex + i2c_ds1307_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > i2c_ds1307_I2C_FIFO_SIZE)
                            {
                                diffCount = i2c_ds1307_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    i2c_ds1307_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                i2c_ds1307_mstrRdBufPtr[i2c_ds1307_mstrRdBufIndex] = (uint8)
                                                                                        i2c_ds1307_RX_FIFO_RD_REG;
                                i2c_ds1307_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(i2c_ds1307_CHECK_INTR_RX_MASKED(i2c_ds1307_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            i2c_ds1307_mstrRdBufPtr[i2c_ds1307_mstrRdBufIndex] = (uint8) i2c_ds1307_RX_FIFO_RD_REG;
                            i2c_ds1307_mstrRdBufIndex++;

                            if(i2c_ds1307_mstrRdBufIndex < i2c_ds1307_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(i2c_ds1307_CHECK_INTR_MASTER_MASKED(i2c_ds1307_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            i2c_ds1307_mstrWrBufIndexTmp -= (i2c_ds1307_GET_TX_FIFO_ENTRIES +
                                                                   i2c_ds1307_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            i2c_ds1307_mstrWrBufIndex = i2c_ds1307_mstrWrBufIndexTmp;

                            i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_ERR_XFER |
                                                                     i2c_ds1307_I2C_MSTAT_ERR_SHORT_XFER);

                            i2c_ds1307_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(i2c_ds1307_CHECK_INTR_TX_MASKED(i2c_ds1307_INTR_TX_EMPTY))
                        {
                            while(i2c_ds1307_I2C_FIFO_SIZE != i2c_ds1307_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(i2c_ds1307_mstrWrBufIndexTmp < i2c_ds1307_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(i2c_ds1307_mstrWrBufIndexTmp == (i2c_ds1307_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    i2c_ds1307_TX_FIFO_WR_REG = (uint32) i2c_ds1307_mstrWrBufPtr[i2c_ds1307_mstrWrBufIndexTmp];
                                    i2c_ds1307_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            i2c_ds1307_ClearTxInterruptSource(i2c_ds1307_INTR_TX_EMPTY);
                        #endif /* (i2c_ds1307_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(i2c_ds1307_CHECK_INTR_TX_MASKED(i2c_ds1307_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            i2c_ds1307_mstrWrBufIndex = i2c_ds1307_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(i2c_ds1307_CHECK_I2C_MODE_NO_STOP(i2c_ds1307_mstrControl))
                    {
                        ...
                        i2c_ds1307_mstrStatus |= (uint16) (i2c_ds1307_I2C_MSTAT_XFER_HALT |
                                                                 i2c_ds1307_GET_I2C_MSTAT_CMPLT);

                        i2c_ds1307_state = i2c_ds1307_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
        ...
        } /* (i2c_ds1307_I2C_MASTER) */
        #endif

    } /* (i2c_ds1307_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(i2c_ds1307_CHECK_I2C_FSM_SLAVE)
    {
        #if(i2c_ds1307_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(i2c_ds1307_CHECK_INTR_SLAVE_MASKED(i2c_ds1307_INTR_SLAVE_I2C_NACK))
            {
                i2c_ds1307_ClearSlaveInterruptSource(i2c_ds1307_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (i2c_ds1307_GET_TX_FIFO_ENTRIES + i2c_ds1307_GET_TX_FIFO_SR_VALID);

                if(i2c_ds1307_slOverFlowCount > diffCount) /* Overflow */
                {
                    i2c_ds1307_slStatus |= (uint8) i2c_ds1307_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    i2c_ds1307_slRdBufIndexTmp -= (diffCount - i2c_ds1307_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                i2c_ds1307_slRdBufIndex = i2c_ds1307_slRdBufIndexTmp;
                ...
                i2c_ds1307_slStatus &= (uint8) ~i2c_ds1307_I2C_SSTAT_RD_BUSY;
                i2c_ds1307_slStatus |= (uint8)  i2c_ds1307_I2C_SSTAT_RD_CMPLT;
                i2c_ds1307_state     =  i2c_ds1307_I2C_FSM_IDLE;
                
            #ifdef i2c_ds1307_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                i2c_ds1307_I2C_SlaveCompleteCallback();
            #endif /* i2c_ds1307_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(i2c_ds1307_CHECK_INTR_SLAVE_MASKED(i2c_ds1307_INTR_SLAVE_I2C_WRITE_STOP))
            {
                i2c_ds1307_ClearSlaveInterruptSource(i2c_ds1307_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (i2c_ds1307_I2C_CTRL_REG & i2c_ds1307_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != i2c_ds1307_GET_RX_FIFO_ENTRIES)
                    {
                        #if(i2c_ds1307_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == i2c_ds1307_GET_RX_FIFO_ENTRIES) &&
                               (i2c_ds1307_CHECK_INTR_SLAVE_MASKED(i2c_ds1307_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        i2c_ds1307_slWrBufPtr[i2c_ds1307_slWrBufIndex] = (uint8) i2c_ds1307_RX_FIFO_RD_REG;
                        i2c_ds1307_slWrBufIndex++;
                    }

                    i2c_ds1307_DISABLE_SLAVE_AUTO_DATA;
                }

                if(i2c_ds1307_CHECK_INTR_RX(i2c_ds1307_INTR_RX_OVERFLOW))
                {
                    i2c_ds1307_slStatus |= (uint8) i2c_ds1307_I2C_SSTAT_WR_OVFL;
                }
                ...
                i2c_ds1307_slStatus &= (uint8) ~i2c_ds1307_I2C_SSTAT_WR_BUSY;
                i2c_ds1307_slStatus |= (uint8)  i2c_ds1307_I2C_SSTAT_WR_CMPLT;
                i2c_ds1307_state     =  i2c_ds1307_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (i2c_ds1307_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                i2c_ds1307_ClearI2CExtClkInterruptSource(i2c_ds1307_INTR_I2C_EC_WAKE_UP);

                #if (i2c_ds1307_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != i2c_ds1307_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = i2c_ds1307_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) i2c_ds1307_RX_FIFO_RD_REG;
                        response = i2c_ds1307_I2C_ACK_ADDR;
                    }
            ...
            #if (i2c_ds1307_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == i2c_ds1307_I2C_NAK_ADDR)
                {
                #if (!i2c_ds1307_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    i2c_ds1307_DISABLE_INTR_SLAVE(i2c_ds1307_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!i2c_ds1307_CY_SCBIP_V0) */
                    ...
                    i2c_ds1307_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (i2c_ds1307_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        i2c_ds1307_slRdBufIndexTmp = i2c_ds1307_slRdBufIndex;

                        /* Start master reading */
                        i2c_ds1307_slStatus |= (uint8) i2c_ds1307_I2C_SSTAT_RD_BUSY;
                        i2c_ds1307_state     = i2c_ds1307_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (i2c_ds1307_slWrBufSize - i2c_ds1307_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == i2c_ds1307_I2C_FIFO_SIZE)
                            {
                                ...
                                i2c_ds1307_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                i2c_ds1307_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        i2c_ds1307_slStatus |= (uint8) i2c_ds1307_I2C_SSTAT_WR_BUSY;
                        i2c_ds1307_state     = i2c_ds1307_I2C_FSM_SL_WR;
                    }
                    ...
                    i2c_ds1307_ENABLE_INTR_SLAVE(i2c_ds1307_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!i2c_ds1307_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    i2c_ds1307_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(i2c_ds1307_CHECK_INTR_RX_MASKED(i2c_ds1307_INTR_RX_FULL))
            {
                ...
                diffCount =  i2c_ds1307_slWrBufSize -
                            (i2c_ds1307_slWrBufIndex + i2c_ds1307_I2C_FIFO_SIZE);

                if(diffCount > i2c_ds1307_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = i2c_ds1307_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    i2c_ds1307_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                i2c_ds1307_ClearRxInterruptSource(i2c_ds1307_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = i2c_ds1307_RX_FIFO_RD_REG;

                if(i2c_ds1307_slWrBufIndex < i2c_ds1307_slWrBufSize)
                {
                    i2c_ds1307_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    i2c_ds1307_slWrBufPtr[i2c_ds1307_slWrBufIndex] = (uint8) diffCount;
                    i2c_ds1307_slWrBufIndex++;
                }
                ...
                i2c_ds1307_ClearRxInterruptSource(i2c_ds1307_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(i2c_ds1307_CHECK_INTR_TX_MASKED(i2c_ds1307_INTR_TX_EMPTY))
            {
                while(i2c_ds1307_I2C_FIFO_SIZE != i2c_ds1307_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(i2c_ds1307_slRdBufIndexTmp < i2c_ds1307_slRdBufSize)
                    /* Data from buffer */
                    {
                        i2c_ds1307_TX_FIFO_WR_REG = (uint32) i2c_ds1307_slRdBufPtr[i2c_ds1307_slRdBufIndexTmp];
                        i2c_ds1307_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        i2c_ds1307_TX_FIFO_WR_REG = i2c_ds1307_I2C_SLAVE_OVFL_RETURN;

                        if(i2c_ds1307_slOverFlowCount <= i2c_ds1307_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            i2c_ds1307_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef i2c_ds1307_I2C_ISR_EXIT_CALLBACK
    i2c_ds1307_I2C_ISR_ExitCallback();
#endif /* i2c_ds1307_I2C_ISR_EXIT_CALLBACK */
