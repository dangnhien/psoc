...
/* Master variables */
volatile uint16 i2c_ds1307_mstrStatus;      /* Master Status byte  */
volatile uint8  i2c_ds1307_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * i2c_ds1307_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  i2c_ds1307_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  i2c_ds1307_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * i2c_ds1307_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  i2c_ds1307_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  i2c_ds1307_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  i2c_ds1307_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!i2c_ds1307_CY_SCBIP_V0 && \
    i2c_ds1307_I2C_MULTI_MASTER_SLAVE_CONST && i2c_ds1307_I2C_WAKE_ENABLE_CONST)
    static void i2c_ds1307_I2CMasterDisableEcAm(void);
#else
    #define i2c_ds1307_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!i2c_ds1307_CY_SCBIP_V0) */

static uint32 i2c_ds1307_I2CMasterHandleStatus(uint32 status);
static uint32 i2c_ds1307_I2CMasterWaitOneUnit(uint32 *timeout);


/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears i2c_ds1307_I2C_MSTAT_WR_CMPLT
*  status.
*
...
*  - i2c_ds1307_I2C_MODE_COMPLETE_XFER - Perform complete transfer
*    from Start to Stop.
*  - i2c_ds1307_I2C_MODE_REPEAT_START - Send Repeat Start instead
*    of Start. A Stop is generated after transfer is completed unless
*    NO_STOP is specified.
*  - i2c_ds1307_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - i2c_ds1307_I2C_MSTR_NO_ERROR - Function complete without error.
*    The master started the transfer.
*  - i2c_ds1307_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - i2c_ds1307_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*
* \globalvars
*  i2c_ds1307_mstrStatus  - used to store current status of I2C Master.
*  i2c_ds1307_state       - used to store current state of software FSM.
*  i2c_ds1307_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  i2c_ds1307_mstrWrBufPtr - used to store pointer to master write buffer.
*  i2c_ds1307_mstrWrBufIndex - used to current index within master write
*  buffer.
*  i2c_ds1307_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 i2c_ds1307_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = i2c_ds1307_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(i2c_ds1307_CHECK_I2C_FSM_IDLE)
        {
            i2c_ds1307_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = i2c_ds1307_CHECK_I2C_STATUS(i2c_ds1307_I2C_STATUS_BUS_BUSY) ?
                            i2c_ds1307_I2C_MSTR_BUS_BUSY : i2c_ds1307_I2C_MSTR_NO_ERROR;
        }
        else if(i2c_ds1307_CHECK_I2C_FSM_HALT)
        {
            i2c_ds1307_mstrStatus &= (uint16) ~i2c_ds1307_I2C_MSTAT_XFER_HALT;
                              errStatus  = i2c_ds1307_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(i2c_ds1307_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        /* Non-empty for master-slave mode when wakeup enabled */
        i2c_ds1307_I2CMasterDisableEcAm();

        /* Set up write transaction */
        i2c_ds1307_state = i2c_ds1307_I2C_FSM_MSTR_WR_ADDR;
        i2c_ds1307_mstrWrBufIndexTmp = 0u;
        i2c_ds1307_mstrWrBufIndex    = 0u;
        i2c_ds1307_mstrWrBufSize     = cnt;
        i2c_ds1307_mstrWrBufPtr      = (volatile uint8 *) wrData;
        i2c_ds1307_mstrControl       = (uint8) mode;

        slaveAddress = i2c_ds1307_GET_I2C_8BIT_ADDRESS(slaveAddress);

        i2c_ds1307_mstrStatus &= (uint16) ~i2c_ds1307_I2C_MSTAT_WR_CMPLT;

        i2c_ds1307_ClearTxInterruptSource    (i2c_ds1307_INTR_TX_UNDERFLOW);
        i2c_ds1307_ClearMasterInterruptSource(i2c_ds1307_INTR_MASTER_ALL);
        i2c_ds1307_SetMasterInterruptMode    (i2c_ds1307_I2C_INTR_MASTER_MASK);

        /* The TX and RX FIFO have to be EMPTY */

        /* Enable interrupt source to catch when address is sent */
        i2c_ds1307_SetTxInterruptMode(i2c_ds1307_INTR_TX_UNDERFLOW);

        /* Generate Start or ReStart */
        if(i2c_ds1307_CHECK_I2C_MODE_RESTART(mode))
        {
            i2c_ds1307_I2C_MASTER_GENERATE_RESTART;
            i2c_ds1307_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            i2c_ds1307_TX_FIFO_WR_REG = slaveAddress;
            i2c_ds1307_I2C_MASTER_GENERATE_START;
        }
    }

    i2c_ds1307_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears i2c_ds1307_I2C_MSTAT_RD_CMPLT
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See i2c_ds1307_I2CMasterWriteBuf()
*  function for constants.
*
* \return
*  Error status.See i2c_ds1307_I2CMasterWriteBuf()
*  function for constants.
*
* \globalvars
*  i2c_ds1307_mstrStatus  - used to store current status of I2C Master.
...
*  i2c_ds1307_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  i2c_ds1307_mstrRdBufPtr - used to store pointer to master read buffer.
*  i2c_ds1307_mstrRdBufIndex - used to current index within master read
*  buffer.
*  i2c_ds1307_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 i2c_ds1307_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(i2c_ds1307_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        i2c_ds1307_state = i2c_ds1307_I2C_FSM_MSTR_RD_ADDR;
        i2c_ds1307_mstrRdBufIndex = 0u;
        i2c_ds1307_mstrRdBufSize  = cnt;
        i2c_ds1307_mstrRdBufPtr   = (volatile uint8 *) rdData;
        i2c_ds1307_mstrControl    = (uint8) mode;

        slaveAddress = (i2c_ds1307_GET_I2C_8BIT_ADDRESS(slaveAddress) | i2c_ds1307_I2C_READ_FLAG);

        i2c_ds1307_mstrStatus &= (uint16) ~i2c_ds1307_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(i2c_ds1307_mstrRdBufSize < i2c_ds1307_I2C_FIFO_SIZE)
        {
            /* Reading byte-by-byte */
            i2c_ds1307_SetRxInterruptMode(i2c_ds1307_INTR_RX_NOT_EMPTY);
        }
        else
        {
            /* Receive RX FIFO chunks */
            i2c_ds1307_ENABLE_MASTER_AUTO_DATA_ACK;
            i2c_ds1307_SetRxInterruptMode(i2c_ds1307_INTR_RX_FULL);
        }
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterSendStart
****************************************************************************//**
*
...
* address byte.
*  - i2c_ds1307_I2C_WRITE_XFER_MODE - Set write direction for the
*    following transfer.
*  - i2c_ds1307_I2C_READ_XFER_MODE - Set read direction for the
*    following transfer.
*
* \param timeoutMs
...
* Error status.
*  - i2c_ds1307_I2C_MSTR_NO_ERROR - Function complete without error.
*  - i2c_ds1307_I2C_MSTR_BUS_BUSY - Bus is busy.
*    Nothing was sent on the bus. The attempt has to be retried.
*  - i2c_ds1307_I2C_MSTR_NOT_READY - Master is not ready for to
*    start transfer.
...
*    Nothing was sent on the bus. The attempt has to be retried.
*  - i2c_ds1307_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was
*    NAKed.
*  - i2c_ds1307_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost
*    arbitration.
*  - i2c_ds1307_I2C_MSTR_ERR_BUS_ERR - Error condition: Master
*    encountered a bus error. Bus error is misplaced start or stop detection.
*  - i2c_ds1307_I2C_MSTR_ERR_ABORT_START - Error condition: The start
...
{
    uint32 errStatus = i2c_ds1307_I2C_MSTR_NOT_READY;

    if(i2c_ds1307_CHECK_I2C_FSM_IDLE)
    {
        i2c_ds1307_DisableInt();  /* Lock from interruption */

        if(i2c_ds1307_CHECK_I2C_STATUS(i2c_ds1307_I2C_STATUS_BUS_BUSY))
        {
            errStatus = i2c_ds1307_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            uint32 locStatus;
            uint32 timeout = i2c_ds1307_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            i2c_ds1307_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            i2c_ds1307_SetMasterInterruptMode    (i2c_ds1307_NO_INTR_SOURCES);
            i2c_ds1307_ClearMasterInterruptSource(i2c_ds1307_INTR_MASTER_ALL);
            ...
            {
                i2c_ds1307_state = i2c_ds1307_I2C_FSM_MSTR_WR_DATA;
            }
            else             /* Read direction */
            {
                i2c_ds1307_state = i2c_ds1307_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= i2c_ds1307_I2C_READ_FLAG;
            }
            ...
            do
            {
                locStatus  = i2c_ds1307_GetMasterInterruptSource() & i2c_ds1307_INTR_MASTER_SEND_BYTE;
                locStatus |= i2c_ds1307_GetSlaveInterruptSource() & i2c_ds1307_SLAVE_INTR_I2C_ADDR;
                locStatus |= i2c_ds1307_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = i2c_ds1307_I2CMasterHandleStatus(locStatus);
        }
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterSendRestart
****************************************************************************//**
*
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See i2c_ds1307_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
* \return
* Error status.
* See i2c_ds1307_I2CMasterSendStart() function for constants.
*
* \sideeffect
...
uint32 i2c_ds1307_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW, uint32 timeoutMs)
{
    ...
    if(i2c_ds1307_CHECK_I2C_MASTER_ACTIVE)
    {
        uint32 locStatus;
        uint32 timeout = i2c_ds1307_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterSendStop
****************************************************************************//**
*
...
uint32 i2c_ds1307_I2CMasterSendStop(uint32 timeoutMs)
{
    ...
    if(i2c_ds1307_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        i2c_ds1307_I2C_MASTER_GENERATE_STOP;

        /* Wait for a completion event from the master */
        do
        {
            locStatus  = i2c_ds1307_GetMasterInterruptSource() & i2c_ds1307_INTR_MASTER_SEND_STOP;
            locStatus |= i2c_ds1307_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterWriteByte
****************************************************************************//**
*
...
uint32 i2c_ds1307_I2CMasterWriteByte(uint32 wrByte, uint32 timeoutMs)
{
    ...
    if(i2c_ds1307_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        i2c_ds1307_TX_FIFO_WR_REG = wrByte;
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterReadByte
****************************************************************************//**
*
...
* i2c_ds1307_I2CMasterSendStop() or
* i2c_ds1307_I2CMasterSendRestart() function appropriately.
* This function is blocking. It does not return until a byte is received or
* an error occurs.
*
* \param ackNack
* Response to received byte.
*  - i2c_ds1307_I2C_ACK_DATA - Generates ACK.
*     The master notifies slave that transfer continues.
*  - i2c_ds1307_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 i2c_ds1307_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    ...
    if(i2c_ds1307_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = i2c_ds1307_GetMasterInterruptSource() & i2c_ds1307_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = i2c_ds1307_GetRxInterruptSource() & i2c_ds1307_INTR_RX_NOT_EMPTY;
            locStatus |= i2c_ds1307_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)i2c_ds1307_RX_FIFO_RD_REG;
            i2c_ds1307_ClearRxInterruptSource(i2c_ds1307_INTR_RX_ALL);
        }
        ...
        if (i2c_ds1307_I2C_MSTR_NO_ERROR == errStatus)
        {
            if(i2c_ds1307_I2C_ACK_DATA == ackNack)
            {
                i2c_ds1307_I2C_MASTER_GENERATE_ACK;
            }
            else
            {
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  i2c_ds1307_I2CMasterReadBuf() function.
*
* \return
...
*  i2c_ds1307_I2C_MSTAT_ERR_ARB_LOST or
*  i2c_ds1307_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  i2c_ds1307_mstrRdBufIndex - used to current index within master read
*  buffer.
...
{
    return(i2c_ds1307_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
{
    return(i2c_ds1307_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterStatus
****************************************************************************//**
*
...
*  - i2c_ds1307_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that
*    read transfer was completed successfully.
*  - i2c_ds1307_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write
*    transfer was completed successfully.
*  - i2c_ds1307_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - i2c_ds1307_I2C_MSTAT_XFER_HALT - Transfer has been halted.
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - i2c_ds1307_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write
*    transfer completed before all bytes were transferred. The slave NAKed
*    the byte which was expected to be ACKed.
*  - i2c_ds1307_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did
*    not acknowledge address.
*  - i2c_ds1307_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost
*    arbitration during communications with slave.
*  - i2c_ds1307_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error
*    occurred during master transfer due to misplaced Start or Stop
*    condition on the bus.
*  - i2c_ds1307_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was
...
*  - i2c_ds1307_I2C_MSTAT_ERR_XFER - Error condition: This is the
...
uint32 i2c_ds1307_I2CMasterStatus(void)
{
    ...
    status = (uint32) i2c_ds1307_mstrStatus;

    if (i2c_ds1307_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) i2c_ds1307_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the i2c_ds1307_I2CMasterStatus()
*  function for constants.
*
* \globalvars
*  i2c_ds1307_mstrStatus - used to store current status of I2C Master.
...
uint32 i2c_ds1307_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) i2c_ds1307_mstrStatus;
    i2c_ds1307_mstrStatus = i2c_ds1307_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CReStartGeneration
****************************************************************************//**
*
...
void i2c_ds1307_I2CReStartGeneration(void)
{
    ...
    * In case of timeout the i2c_ds1307_I2CMasterSendRestart handles this
    * case using own timeout and execute appropriate recovery.
    */

    const uint16 oneUnit = 1u;
    uint32 timeout = i2c_ds1307_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = i2c_ds1307_I2C_STATUS_REG & i2c_ds1307_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((i2c_ds1307_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = i2c_ds1307_I2C_PHASE_GEN_TIMEOUT;

        /* Drive SCL line use GPIO */
        i2c_ds1307_SET_I2C_SCL_DR       (i2c_ds1307_I2C_SCL_LOW);
        i2c_ds1307_SET_I2C_SCL_HSIOM_SEL(i2c_ds1307_HSIOM_GPIO_SEL);
        ...
        if (0u != readDirection)
        {
            ...
            i2c_ds1307_SET_I2C_SCL_DR(i2c_ds1307_I2C_SCL_LOW);
            CyDelayUs(i2c_ds1307_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            i2c_ds1307_SET_I2C_SCL_DR(i2c_ds1307_I2C_SCL_HIGH);
            CyDelayUs(i2c_ds1307_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((i2c_ds1307_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
        ...
        if (timeout > 0u)
        {
            ...
            i2c_ds1307_I2C_MASTER_CMD_REG  = i2c_ds1307_I2C_MASTER_CMD_M_START;
        }
        ...
        */
        ...
        i2c_ds1307_SET_I2C_SCL_HSIOM_SEL(i2c_ds1307_HSIOM_I2C_SEL);
        i2c_ds1307_SET_I2C_SCL_DR       (i2c_ds1307_I2C_SCL_HIGH);
    }
#else
    uint32 cmd;
    ...
    cmd  = i2c_ds1307_I2C_MASTER_CMD_M_START;
    cmd |= i2c_ds1307_CHECK_I2C_STATUS(i2c_ds1307_I2C_STATUS_M_READ) ?
                (i2c_ds1307_I2C_MASTER_CMD_M_NACK) : (0u);

    i2c_ds1307_I2C_MASTER_CMD_REG = cmd;
    ...
    i2c_ds1307_I2C_MULTI_MASTER_SLAVE_CONST && i2c_ds1307_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: i2c_ds1307_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    static void i2c_ds1307_I2CMasterDisableEcAm(void)
    {
        ...
        */
        if (0u != (i2c_ds1307_CTRL_REG & i2c_ds1307_CTRL_EC_AM_MODE))
        {
            ...
            i2c_ds1307_Stop();
            i2c_ds1307_CTRL_REG &= (uint32) ~i2c_ds1307_CTRL_EC_AM_MODE;
            i2c_ds1307_Enable();
        }
    }
#endif /* (!i2c_ds1307_CY_SCBIP_V0) */
...
/*******************************************************************************
* Function Name: i2c_ds1307_I2CMasterHandleStatus
****************************************************************************//**
*
...
* i2c_ds1307_I2C_MASTER_TIMEOUT and i2c_ds1307_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See i2c_ds1307_I2CMasterSendStart() function for constants.
...
static uint32 i2c_ds1307_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & i2c_ds1307_I2C_MASTER_TIMEOUT))
    {
        retStatus  = i2c_ds1307_I2C_MASTER_TIMEOUT;
        resetBlock = i2c_ds1307_I2C_RESET_ERROR;
    }
    else if (0u != (i2c_ds1307_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = i2c_ds1307_I2C_MSTR_ERR_ABORT_START;

        i2c_ds1307_I2C_MASTER_CMD_REG = 0u;

        i2c_ds1307_state = i2c_ds1307_I2C_FSM_IDLE;
    }
    else if (0u != (status & i2c_ds1307_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = i2c_ds1307_I2C_MSTR_ERR_ARB_LOST;

        if (i2c_ds1307_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (i2c_ds1307_I2C_CTRL_REG & i2c_ds1307_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = i2c_ds1307_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & i2c_ds1307_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = i2c_ds1307_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = i2c_ds1307_I2C_RESET_ERROR;
    }
    else if (0u != (status & i2c_ds1307_INTR_MASTER_I2C_NACK))
    {
        retStatus = i2c_ds1307_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = i2c_ds1307_I2C_MSTR_NO_ERROR;

        if (0u != (status & i2c_ds1307_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (i2c_ds1307_CHECK_I2C_FSM_ADDR)
            {
                ...
                i2c_ds1307_state = (i2c_ds1307_CHECK_I2C_FSM_RD) ?
                                          i2c_ds1307_I2C_FSM_MSTR_RD_DATA : i2c_ds1307_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (i2c_ds1307_I2C_RESET_ERROR == resetBlock)
    {
        i2c_ds1307_I2CFwBlockReset();
    }
    else
    {
        i2c_ds1307_ClearMasterInterruptSource(i2c_ds1307_INTR_MASTER_ALL);
...
static uint32 i2c_ds1307_I2CMasterWaitOneUnit(uint32 *timeout)
{
    ...
    if (*timeout > 0u)
    {
        ...
        if (0u == *timeout)
        {
            status = i2c_ds1307_I2C_MASTER_TIMEOUT;
        }
    }
