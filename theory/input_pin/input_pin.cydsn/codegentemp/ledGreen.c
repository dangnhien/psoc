...
#include "ledGreen.h"
    ...
    #define ledGreen_SetP4PinDriveMode(shift, mode)  \
    do { \
        ledGreen_PC =   (ledGreen_PC & \
                                (uint32)(~(uint32)(ledGreen_DRIVE_MODE_IND_MASK << \
                                (ledGreen_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (ledGreen_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define ledGreen_SetP4PinDriveMode(shift, mode)  \
        do { \
            ledGreen_USBIO_CTRL_REG = (ledGreen_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(ledGreen_DRIVE_MODE_IND_MASK << \
                                    (ledGreen_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (ledGreen_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		ledGreen_SetP4PinDriveMode(ledGreen__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(ledGreen_DR & (uint8)(~ledGreen_MASK));
    drVal = (drVal | ((uint8)(value << ledGreen_SHIFT) & ledGreen_MASK));
    ledGreen_DR = (uint32)drVal;
}
...
{
    return (uint8)((ledGreen_PS & ledGreen_MASK) >> ledGreen_SHIFT);
}
...
{
    return (uint8)((ledGreen_DR & ledGreen_MASK) >> ledGreen_SHIFT);
}
...
void ledGreen_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  ledGreen_INTCFG & (uint32)(~(uint32)position);
    ledGreen_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(ledGreen_INTSTAT & ledGreen_MASK);
	ledGreen_INTSTAT = maskedStatus;
    return maskedStatus >> ledGreen_SHIFT;
}
